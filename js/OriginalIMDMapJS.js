...
if (Modernizr.webgl) {
  ...
  d3.queue().defer(d3.json, 'data/config.json').await(ready)
  ...
  function ready (error, config) {
      ...
      map.addLayer(
        {
          ...
          source: {
            ...
            tiles: [
              'https://cdn.ons.gov.uk/maptiles/t18/tiles/{z}/{x}/{y}.pbf'
            ],
            minzoom: 4,
            maxzoom: 13
          },
      ...
      map.addLayer(
        {
          ...
          source: {
            type: 'vector',
            tiles: [
              'https://cdn.ons.gov.uk/maptiles/t18/tiles2/{z}/{x}/{y}.pbf'
            ],
            minzoom: 10
          },
    ...
    function onMove (e) {
      ...
      if (newlsoa11cd != oldlsoa11cd) {
        ...
        map.setFilter('lsoa-outlines-hover', [
          '==',
          'lsoa11cd',
          e.features[0].properties.lsoa11cd
        ])
        ...
        if (features.length != 0) {
          setAxisVal(
            features[0].properties.lsoa11nm,
            features[0].properties[hoverlayername]
          )
    ...
    function onClick (e) {
      ...
      if (newlsoa11cd != oldlsoa11cd) {
        ...
        updatePercent(e.features[0])
        setAxisVal(
          e.features[0].properties.lsoa11nm,
          e.features[0].properties[hoverlayername]
        )
      }
    ...
    function createKey (config) {
      ...
      var svgkey = d3.select('#key').attr('width', keywidth).attr('height', 20)
      ...
      d3.selectAll('.tick text').attr(
        'transform',
        'translate(-' + x.range()[1] / 10 / 2 + ',0)'
      )
    ...
    function createLegend (keydata) {
      ...
      legend = d3
        .select('#details-content-3') //.append('ul')
        // 	.attr('class', 'key')
        .selectAll('g')
        ...
        .attr('class', function (d, i) {
          return 'key-item key-' + i + ' b ' + d.replace(' ', '-').toLowerCase()
        })
      ...
      d3.selectAll('.legendRect1')
        ...
        .style(
          'background',
          'repeating-linear-gradient(to right, #666, #666 ' +
            ((barwidth - 20) / 10 - 2) +
            'px, #000 2px, #000 ' +
            (barwidth - 20) / 10 +
            'px)'
    ...
    function updatePercent (props) {
      ...
      percentages.forEach(function (d, i) {
        d3.select('#legendRect' + i)
          .transition()
          .duration(300)
          .style('width', (barwidth - 20) / 10 - 2 + 'px')
          .style('left', (percentages[i] - 1) * ((barwidth - 20) / 10) + 'px')
      })
    }
        ...
        var tilechecker = setInterval(function () {
          ...
          if (features.length != 0) {
            ...
            map.setFilter('lsoa-outlines-hover', [
              '==',
              'lsoa11cd',
              features[0].properties.lsoa11cd
            ])
    ...
    function selectlist (datacsv) {
      ...
      $('#areaselect')
        .chosen({ width: '98%', allow_single_deselect: true })
